...
def get_silhouette_scores(data, init, max_iter=20, num_runs=10):
    ...
    for num_iterations in range(1, max_iter+1):
        ...
        for run in range(num_runs):
            kmeans_plusplus = KMeans(n_clusters=k, init='k-means++', max_iter=num_iterations).fit(data_set)
            scores.append(silhouette_score(data_set, kmeans_plusplus.labels_))
...
sub.errorbar(iter, avg_plus, yerr=std_plus, fmt='b+')

sub = fig.add_subplot(222)
sub.set_title('Random sampled')
sub.errorbar(iter, avg_random, yerr=std_random, fmt='r+')
